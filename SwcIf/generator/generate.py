...
def load_component(path: Path):
    ...
    if d.get("client_bindings") is not None:
        ...
        for i in d["client_bindings"].get("prototypes", []):
            base_name, array_size = parse_array_name(i["name"])
            prototypes.append(
                ClientSpec(
                    name=base_name,
                    array_size=array_size,
                    return_type=i["return_type"],
                    args=load_args(i.get("args", [])),
                )
            )
        cb = ClientBindingsSpec(name=d["client_bindings"]["name"], prototypes=prototypes)
    servers = [
    ...
    for inst in ecu.instances:
        ...
        for proto in (comp.client_bindings.prototypes if comp.client_bindings else []):
            if proto.array_size is None:
                ...
                items = []
                for idx in range(proto.array_size):
                    items.append({
                        "index": idx,
                        "bind_expr": inst.client_bindings.get(f"{proto.name}[{idx}]"),
                    })
                bindings.append({
                    "prototype": proto,
                    "items": items,
...
def render_all(root: Path, ecu_yaml: Path, out_dir: Path, *, emit_swc_templates: bool = False, swc_templates_out: Path = None):
    ...
    for inst in ctx["instances"]:
        rendered = env.get_template("SwcIf_Bind_Instance.stub.c.j2").render(instance=inst).rstrip("\n") + "\n\n"
        (out_dir / f"SwcIf_Bind_{inst['name']}.stub.c").write_text(rendered, encoding="utf-8")
...
if __name__ == "__main__":
    ...
    render_all(
        Path(a.root).resolve(),
        Path(a.root).resolve() / a.ecu,
        Path(a.root).resolve() / a.out,
        emit_swc_templates=a.emit_swc_templates,
        swc_templates_out=(Path(a.swc_templates_out).resolve() if a.swc_templates_out else None),
    )
